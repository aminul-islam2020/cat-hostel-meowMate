    ...
    <div class="max-w-7xl mx-auto px-4 pb-20 grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden mb-4">
                <img id="main-image" src="<?php echo $provider['facility_photos'][0]; ?>"
                    alt="<?php echo $provider['full_name']; ?>" class="w-full h-96 object-cover">
            </div>
            <div class="grid grid-cols-3 gap-4">
                <?php foreach ($provider['facility_photos'] as $index => $img): ?>
                    <button class="thumbnail-btn" data-image="<?php echo $img; ?>">
                        <img src="<?php echo $img; ?>" alt="Photo <?php echo $index + 1; ?>"
        ...
        <div>
            ...
            <p class="text-gray-600 mb-2"><strong>Experience:</strong> <?php echo $provider['experience_years']; ?>
                years</p>
            ...
            <div class="flex gap-4">
                <button onclick="contactProvider(<?php echo $provider['id']; ?>)"
                    class="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700">Contact</button>
                <button onclick="bookProvider(<?php echo $provider['id']; ?>)"
                    class="bg-green-600 text-white px-6 py-3 rounded-lg hover:bg-green-700">Book Now</button>
            </div>
        </div>
    </div>
